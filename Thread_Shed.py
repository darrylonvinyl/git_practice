...
daily_transactions_split = [record.split('_') for record in daily_transactions]
...
def populate_customer_data(customers_index,sales_index,thread_sold_index):
  for x in range(customers_index,len(transactions_clean),4):
    customers.append(transactions_clean[x])
  for x in range(sales_index,len(transactions_clean),4):
    sales.append(transactions_clean[x])
  for x in range(thread_sold_index,len(transactions_clean),4):
    thread_sold.append(transactions_clean[x])

populate_customer_data(customers_index,sales_index,thread_sold_index)
# print(customers)
# print(sales)
# print(thread_sold)
...
for color in colors:
  print('We sold {number} threads of {color} today.'.format(number=color_count(color),color=color))
